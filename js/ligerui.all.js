...
(function($) {
    ...
    liger.core.UIComponent.ligerExtend(liger.core.Component, {
        ...
        _init: function() {
            ...
            //权限控制
            if (!p.permission && window.page.permission) {
                p.permission = window.page.permission[this.id];
            }
        },
        //预渲染,可以用于继承扩展
        _preRender: function() {
            ...
            return $.extend({
                create: function(container, editParm, controlOptions) {
                    ...
                    if (editParm.field) {
                        ...
                        if ($.inArray(type, ["select", "combobox", "autocomplete", "popup", "selectuser", "selectdep"]) != -1) {
                            txtInputName = field.textField || field.comboboxName;
                            if (field.comboboxName && !field.id)
                                p.id = (options.prefixID || "") + field.comboboxName;
                        }
                        if ($.inArray(type, ["select", "combobox", "autocomplete", "popup", "selectuser", "selectdep", "radiolist", "checkboxlist", "listbox"]) != -1) {
                            p.valueFieldID = prefixID + field.name;
                        }
                        if (!e.body || type == "textarea") {
...
(function($) {
    ...
    $.ligerui.controls.CheckBoxList.ligerExtend($.ligerui.controls.Input, {
        ...
        refresh: function() {
            ...
            var out = [],
                rowSize = p.rowSize,
                appendRowStart = false,
                name = p.name || g.id;
            for (var i = 0; i < data.length; i++) {
                var val = data[i][p.valueField],
                    txt = data[i][p.textField],
                    id = g.id + "-" + i;
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function() {
            ...
            if (p.data && typeof p.data == 'object') {
                g.data = p.data.constructor == Array ? [] : {};
                $.extend(true, g.data, p.data);
            } else {
                g.data = p.data;
            }
        ...
        setTree: function(tree) {
            ...
            if (tree.checkbox != false) {
                ...
                tree.onSelect = function(node) {
                    ...
                    if (p.treeLeafOnly && node.data.children) return;
                    value = node.data[p.valueField];
                    if (p.tree.fullPathText) {
                        var temp = node.data;
                        var path = [];
                        while (temp) {
                            path.unshift(temp[p.textField]);
                            temp = this.getParent(temp);
                        }
                        text = path.join(p.tree.pathSplit);
                    } else {
                        text = node.data[p.textField];
                    }
                    g._changeValue(value, text);
                };
        ...
        setGrid: function(grid) {
            ...
            if (p.condition) {
                ...
                conditionParm.buttons = conditionParm.buttons || [{
                    text: p.Search,
                    width: 40,
                    click: function() {
                        g.grid.query({
                            filters: g.condition.toSearchFilters()
                        });
                    }
                }]
                conditionParm.buttons.push({
                    text: '关闭',
                    width: 40,
                    click: function() {
                        g.selectBox.hide();
            ...
            var /*selecteds = [],*/ onGridSelect = function(select, data) {
                    var value, text, values = [],
                        texts = [];
                    if (grid.checkbox) {
                        var currentValue = g.getValue();
                        if (currentValue) {
                            values = currentValue.split(p.split);
                            texts = g.getText().split(p.split);
                        }

                        $(data).each(function(index, rowdata) {
                            var selectValue = rowdata[p.valueField];
                            if (select) {
                                if ($.inArray(selectValue, values) == -1) {
                                    values.push(selectValue);
                                    texts.push(rowdata[p.textField]);
                                }
                            } else {
                                var i = $.inArray(selectValue, values);
                                if (i >= 0) {
                                    values.splice(i, 1);
                                    texts.splice(i, 1);
                                }
                        ...
                        values.push(data[0][p.valueField]);
                        texts.push(data[0][p.textField]);
                    }

                    //var value = g.getValue().split(p.split), text = g.getText().split(p.split);
                    //$(selecteds).each(function (i, rowdata) {
                    //    var selectValue = rowdata[p.valueField];
                    //    if (!$.inArray(selectValue, value)) {
                    ...
                    //else
                    //    g.selected = null;
                    value = values.join(p.split);
                    text = texts.join(p.split);
                    g._changeValue(value, text);
            ...
            if (grid.checkbox) {
                ...
                g.grid.bind('CheckAllRow', function(checked) {
                    //$(g.grid.rows).each(function (index, rowdata) {
                    //    onCheckRow(checked, rowdata);
                    //});
                    onGridSelect(checked, g.grid.rows);
                });
                g.grid.bind('CheckRow', function(checked, rowdata) {
                    //onCheckRow(checked, rowdata);
                    onGridSelect(checked, [rowdata]);
                });
            } else {
                g.grid.bind('SelectRow', function(rowdata) {
                    if (p.treeLeafOnly && rowdata.__hasChildren) {
                        g.grid.unselect(rowdata);
                        return;
                    };
                    //selecteds = [rowdata];
                    onGridSelect(true, [rowdata]);
                    g._toggleSelectBox(true);
                });
        ...
        _changeValue: function(newValue, newText) {
            ...
            g.inputText.trigger("change").blur().keyup();
            //g.inputText.trigger("change").focus().keyup();
        ...
        _toggleSelectBox: function(isHide) {
            ...
            if (isHide) {
                if (p.slide) {
                    ...
                    $(".l-box-select-inner", g.selectBox).scrollTop(0);
                    g.selectBox.hide();
                    g.boxToggling = false;
                }
            } else {
...
(function($) {
    ...
    $.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
        ...
        _setValue: function(value) {
            ...
            if (dateValue) {
                g.inputText.val(g.getFormatDate(dateValue));
            } else {
                g.inputText.val("");
                g.usedDate = null;
            }
...
(function($) {
    ...
    $.ligerDefaults.Filter.operators['string'] =
        $.ligerDefaults.Filter.operators['text'] = ["equal", "notequal", "startwith", "endwith", "like", "greater", "greaterorequal", "less", "lessorequal", "in", "notin"];

    $.ligerDefaults.Filter.operators['number'] =
        $.ligerDefaults.Filter.operators['int'] =
        $.ligerDefaults.Filter.operators['float'] =
        $.ligerDefaults.Filter.operators['date'] = ["equal", "notequal", "greater", "greaterorequal", "less", "lessorequal", "in", "notin"];
...
(function($) {
    ...
    //        if (editParm.field.height) editor.height(editParm.field.height);
    ...
    //            debugger;
    //            var editor = $('<textarea/>'),
    //                options = $.extend({ readonly: false }, editParm.field.options, { readonly: p.readonly });
    //            var id = (p.prefixID || "") + editParm.field.name;
    //            if ($("#" + id).length) {
    //                editor = $("#" + id);
    //            }
    ...
    //                editor.addClass("l-texterea-readonly");
    //            } else {
    //                editor.addClass("l-texterea");
    //            }
    //            $.extend(editor, {
    //                setEnabled: function () {
    //                    var g = this;
    ...
    $.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function() {
            ...
            if (g.form.is("form")) {
                g.form.attr("onsubmit", "return false")
            }
            //生成ligerui表单样式
            ...
            if (p.buttons && p.buttons.length > 0) {
                //权限初始化
                if (p.permission && p.permission.buttons) {
                    for (var i = 0; i < p.buttons.length; i++) {
                        var id = p.buttons[i].id;
                        if (p.permission.buttons.hasOwnProperty(id) &&
                            p.permission.buttons[id] &&
                            p.permission.buttons[id].right == false) {
                            p.buttons.splice(i--, 1);
                        }
                    }
                }
                if (p.buttons && p.buttons.length > 0) {
        ...
        _preSetFields: function(fields) {
            var g = this,
                ...
                lastVisitedGroupIcon = null;
            //权限初始化
            if (p.permission && p.permission.fields) {
                $(p.fields).each(function(i, field) {
                    var name = field.name;
                    if (p.permission.fields.hasOwnProperty(name) &&
                        p.permission.fields[name] &&
                        p.permission.fields[name].right == false) {
        ...
        _setFields: function(fields) {
            ...
            var jform = $(this.element);
            if (g.editors) {
                for (var indexKey in g.editors) {
                    if (g.editors.hasOwnProperty(indexKey)) {
                        var tempEditor = g.editors[indexKey];
                        if (tempEditor && tempEditor.control && tempEditor.control.destroy) {
                            tempEditor.control.destroy();
                        }
                    }
                }
                g.editors = {};
            ...
            if (fields && fields.length) {
                ...
                if (p.buttonsInline)
                    jform.addClass("l-inline-form-buttons");
            ...
            (function() {
                ...
                $(fields).each(function(fieldIndex, field) {
                    var container = $(g.element).find("[id='" + g.id + "|" + fieldIndex + "']") /*document.getElementById(g.id + "|" + fieldIndex)*/ ,
                        editor = p.editors[field.type];
                    if (!container) return;
                    if (p.onTextKeydown && field.type == "text") {
                        field.options = field.options || {};
                        field.options.onKeydown = field.options.onKeydown || p.onTextKeydown;
                    }
                    container = $(container);
                    var controlWidth = field.width || container.width();
                    var controlHeight = field.height || container.height();
                    ...
                    var readonly = (field.readonly || (field.editor && field.editor.readonly)) ? true : false,
                        txtInputName = (p.prefixID || "") + (field.textField || field.id || field.name);

                    if (g.form && p.validate && g.form.validate && !readonly) {
                        if (editorControl && editorControl.setValidate) {
                            editorControl.setValidate(g.validate, field.validate);
                ...
                (function() {
                    $(fields).each(function(fieldIndex, field) {
                        ...
                        if (name && (name in data)) {
                            var value = g._getValueByName(data, name);
                            editor.editor.setValue(editor.control, value);
                        }
                        if (textField && (textField in data)) {
                            var text = g._getValueByName(data, textField);
            ...
            var validateOptions = $.extend({
                errorPlacement: function(lable, element) {
                    ...
                    if (element.hasClass("l-textarea")) {
                        ...
                        element.parents(".l-controlcontainer:first").addClass("error");
                    }
                    element.parents(".l-controlcontainer:first")
                        .tooltip({ container: "body", placement: "auto right" })
                        .attr("data-original-title", lable.html());
                ...
                success: function(lable, element) {
                    ...
                    //element = $("#" + lable.attr("for"));
                    if ($element.hasClass("l-textarea")) {
                        $element.removeClass("l-textarea-invalid");
                    } else if ($element.hasClass("l-text-field")) {
                        $element.parent().removeClass("l-text-invalid");
                    } else {
                        $element.parents(".l-controlcontainer:first").removeClass("error");
                    }
                    $element.parents(".l-controlcontainer:first").attr("data-original-title", "");
                    //$element.removeAttr("title").ligerHideTip();
                    //if (!lable.attr("for")) return;
        ...
        _buliderControlContainer: function(field, fieldIndex) {
            ...
            out.push(' class="' + (p.fieldCss ? 'l-controlcontainer ' + p.fieldCss : 'l-controlcontainer') + '"');
            out.push(' style="');
            if (width) {
        ...
        _buliderSpaceContainer: function(field) {
            ...
            out.push('">');
            if (field.validate && (field.validate.required || field.validate.markrequired)) {
...
(function($) {
    ...
    $.ligerDefaults.Grid = {
        ...
        */
        ...
        pageSizeOptions: [20, 30, 50], //可选择设定的每页结果数
        parms: [], //提交到服务器的参数
        columns: [], //数据源
        ...
        autoCheckParent: false, //是否自动选中父节点
    ...
    $.ligerDefaults.Grid.formatters['number'] = function(value, column) {
        if (value != null) {
            ...
            if (isNaN(value)) {
                ...
                value = value.toFixed(column.precision || 2);
            }
        }
        return value;
    };
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _init: function() {
            ...
            //初始化权限
            if (p.permission) {
                if (p.toolbar && p.permission.toolbar) {
                    p.toolbar.permission = p.permission.toolbar;
                }
                if (p.permission.columns) {
                    for (var i = 0; i < p.columns.length; i++) {
                        var column = p.columns[i];
                        if (p.permission.columns[column.name] && !p.permission.columns[column.name].right) {
            ...
            if (p.tree) //启用分页模式
            {
                ...
                p.tree.isParent = p.tree.isParent || function(rowData) {
                    //#kad
                    //增加对子数组长度判断
                    var exist = p.tree.childrenName in rowData && rowData[p.tree.childrenName] && rowData[p.tree.childrenName].length > 0;
                    return exist;
                };
        ...
        _render: function() {
            ...
            if (p.minHeight) {
                g.grid.css("min-height", p.minHeight + "px");
            }
            var gridhtmlarr = [];
            ...
            gridhtmlarr.push("                </div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'><span class='pcontrol'> <input type='text' size='4' value='1' style='width:20px' maxlength='9' /> / <span></span></span></div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'>");
        ...
        增加query方法，方便进行查询
        */
        query: function(prams) {
            var g = this;
            var values = $.extend(true, {}, prams);
            g.set('parms', values);
            g.loadData(null, true);
        },
        getQueryParams: function() {
            var g = this,
                p = this.options;
            //还未进行查询
            if (!p.parms || p.parms instanceof Array) return null;

            var param = $.extend(true, { filters: [], sorts: [] }, p.parms);
            if (p.usePager) {
                param.page = p.newPage;
                param.pageSize = p.pageSize;
            }
            if (p.sortName) {
                param.sorts = [{ field: p.sortName, isAsc: p.sortOrder == "asc" }];
                //param.sorts.push({ field: p.sortName, isAsc: p.sortOrder == "asc" })
            }
            return param;
        },
        //刷新数据
        loadData: function(loadDataParm, restPage) {
            var g = this,
                p = this.options;
            g.loading = true;
            g.trigger('loadData');
            ...
            修改返回服务器端的查询参数param
            */
            var param = { filters: [], sorts: [] };
            if (p.dataAction == "server") {
                param = $.extend(true, param, p.parms);

                param.dbKey = p.dbKey;
                param.entityType = p.entityType;
                if (p.usePager) {
                    param.page = p.newPage = restPage ? 1 : p.newPage;
                    param.pageSize = p.pageSize;
                }
                if (p.sortName) {
                    param.sorts = [{ field: p.sortName, isAsc: p.sortOrder == "asc" }];
        ...
        loadServerData: function(param, clause) {
            ...
            //数据、总数分开获取
            if (p.countUrl) {
                loadServerDataId = kad.ui.util.guid();
            }
            ...
            var ajaxOptions = {
                ...
                增加数据Json转化
                */
                data: (kad && kad.ui && kad.ui.util) ? kad.ui.util.stringify(param) : param,
                async: p.async,
                dataType: 'json',
                ...
                success: function(data) {
                    ...
                    if (loadServerDataId) {
                        ...
                        g.loadServerDataId = loadServerDataId;
                        kad.ui.util.request(p.countUrl, param, function(result) {
                            if (g.loadServerDataId == loadServerDataId) {
                                g._showPager(result);
                            }
        ...
        */
        ...
        addRows: function(rowdataArr, neardata, isBefore, parentRowData) {
            ...
            $(rowdataArr).each(function() {
                g.addRow(this, neardata, isBefore, parentRowData, false);
            });
            g.reRender();
        },
        ...
        _addNewRecord: function(o, previd, pid) {
            ...
            if (p.tree) {
                ...
                o['__hasChildren'] = o[p.tree.childrenName] && o[p.tree.childrenName].length ? true : false;
            }
        ...
        addRow: function(rowdata, neardata, isBefore, parentRowData, reRender) {
            var g = this,
                p = this.options;
            if (reRender == null) reRender = true; //默认需要刷新
            rowdata = rowdata || {};
            g._addData(rowdata, parentRowData, neardata, isBefore);
            if (reRender) g.reRender();
            //标识状态
        ...
        reRender: function(e) {
            ...
            if (rowdata && column) {
                ...
                if (!ignoreTotal) {
                    g._reRenderTotalCell(column);
                }
            } else if (rowdata) {
                $(g.columns).each(function() {
                    g.reRender({ rowdata: rowdata, column: this, ignoreTotal: true });
                    g._reRenderTotalCell(this);
                });
            } else if (column) {
                for (var rowid in g.records) {
                    g.reRender({ rowdata: g.records[rowid], column: column, ignoreTotal: true });
                }
                g._reRenderTotalCell(column);
            } else {
                g._showData();
        ...
        _reRenderTotalCell: function(column) {
            var g = this,
                p = this.options;
            if (column.totalSummary) {
                for (var i = 0; i < g.totalNumber; i++) {
                    var tobj = document.getElementById(g.id + "|total" + i + "|" + column['__id']);
                    if (tobj) $("div:first", tobj).html(g._getTotalCellContent(column, g.groups && g.groups[i] ? g.groups[i] : g.currentData[p.root]));
                }
            }
        },
            ...
            if (typeof columnParm == "string") // column id
            {
                if (g._isColumnId(columnParm))
                    ...
                    for (var id in g._columns) {
                        if (g._columns[id].name === columnParm) {
                            return g._columns[id];
                        }
                    }
                }
        ...
        getSelectedRowData: function() {
            ...
            if (row) {
                ...
                data = this.formatRecord(data, true);
                return data;
            }
            return null;
        },
        getSelectedRowsData: function() {
            ...
            for (var i = 0; i < rows.length; i++) {
                var o = $.extend(true, {}, rows[i]);
                data.push(this.formatRecord(o, true));
            }
            return data;
        },
        ...
        _createHeaderCell: function(column) {
            ...
            if (column.editor) {
                $(".l-grid-hd-cell-text:first", jcell).addClass("l-gird-hd-cell-editor");
            }
        ...
        _initFootbar: function() {
            var g = this,
                p = this.options;
            if (p.usePager) {
                g.gridview.css("margin-bottom", "32px");
        ...
        _clearGrid: function() {
            ...
            //取消全选
            if (p.checkbox && g.body) {
                g.body.find('.l-grid-hd-row').removeClass('l-checked');
            }
            //清空数据
            g.gridbody.html("");
            g.f.gridbody.html("");
        ...
        _showData: function() {
            ...
            if (p.tree && p.autoCheckParent) {
                for (var i in g.selected) {
                    g._autoCheckParent(g.selected[i], false);
                }
            }
        },
        _fixHeight: function() {
                ...
                $(g.columns).each(function(columnindex, column) {
                    ...
                    if (this.isrownumber) {
                        ...
                        if (p.fixedCellHeight)
                            gridhtmlarr.push(' style = "white-space: nowrap; text-overflow:ellipsis; height:' + p.rowHeight + 'px;" ');
                        else
        ...
        _getCellHtml: function(rowdata, column) {
            ...
            if (p.fixedCellHeight) htmlarr.push('white-space: nowrap; text-overflow:ellipsis; height:' + p.rowHeight + 'px;');
        ...
        _applyEditor: function(obj) {
            ...
            if (column.editor.type && p.editors[column.editor.type]) {
                ...
                var editor = p.editors[column.editor.type],
                    ...
                    topbarHeight = (p.toolbar ? g.topbar.outerHeight() : 0) + (p.title ? g.header.outerHeight() : 0),
        ...
        _getTotalCellContent: function(column, data) {
            ...
            data = $.grep(data, function(item) {
                return item[p.statusName] !== 'delete';
            });
            ...
            if (column.totalSummary) {
                ...
                var fixed = column.totalSummary.fixed == null ? 2 : column.totalSummary.fixed,
                    realFixed = 0,
                    fixedPow = Math.pow(10, fixed); //解决javascript浮点运算Bug
                var sum = 0,
                    count = 0,
                    avg = 0;
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    realFixed = index >= 0 ? (valueStr.length - index - 1) : 0;

                    if (realFixed > 0 && realFixed < fixed) {
                        sum += parseInt(value * Math.pow(10, realFixed) * Math.pow(10, fixed - realFixed));
                    } else {
                        sum += parseInt(value * fixedPow);
                    }
                }
                sum = sum / fixedPow;
                ...
                if (column.totalSummary.render) {
                    ...
                    if (isExist('sum'))
                        totalsummaryArr.push("<div>合计=" + sum.toFixed(fixed) + "</div>");
                    if (isExist('count'))
                        totalsummaryArr.push("<div>数量=" + count + "</div>");
                    if (isExist('max'))
                        totalsummaryArr.push("<div>最大值=" + max.toFixed(fixed) + "</div>");
                    if (isExist('min'))
                        totalsummaryArr.push("<div>最小值=" + min.toFixed(fixed) + "</div>");
                    if (isExist('avg'))
                        totalsummaryArr.push("<div>平均值=" + avg.toFixed(fixed) + "</div>");
                }
            }
        ...
        _bulidTotalSummary: function(frozen) {
            ...
            if (!g.currentData) return false;
            var data = g.currentData[p.root];
            data = $.grep(data, function(item) {
                return item[p.statusName] !== 'delete';
            });
            if (data.length == 0) return false;
            var totalRow = $(g._getTotalSummaryHtml(data, null, frozen));
        ...
        _onClick: function(e) {
            ...
            } else if (src.row && g.enabledCheckbox()) //复选框选择行
            {
                ...
                if (src.checkbox || !selectRowButtonOnly) {
                    ...
                    g[met](src.data);
                    if (p.tree) {
                        if (p.autoCheckChildren) {
                            var children = g.getChildren(src.data, true);
                            for (var i = 0, l = children.length; i < l; i++) {
                                g[met](children[i]);
                                g.trigger('checkRow', [!uncheck, children[i], children[i]['__id']]);
                            }
                        }
                        if (p.autoCheckParent) {
                            g._autoCheckParent(src.data, uncheck);
                        }
                    }
        ...
        _autoCheckParent: function(rowData, uncheck) {
            var g = this,
                p = this.options,
                parent = g.getParent(rowData),
                items = [];
            while (parent != null) {
                //if (!uncheck) {
                //    var check = false;
                ...
                //            if (g.selected[i] == parent.children[c]) {
                //                check = true;
                //                break;
                //            }
                //        }
            ...
            for (var i in items) {
                var item = items[i];
                g[uncheck ? 'unselect' : 'select'](item);
                g.trigger('checkRow', [!uncheck, item, item['__id']]);
            }
        },
    ...
    $.ligerui.controls.Layout.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function() {
            ...
            if ($("> div[position=top]", g.layout).length > 0) {
                ...
                if (g.topHeight === "auto") {
                    g.top.content.css({ top: "auto", bottom: "auto" });
                } else if (g.topHeight) {
        ...
        _onResize: function() {
            ...
            if (g.top) {
                if (g.topHeight === 'auto') {
                    var topHeight = $('.l-layout-content:first', g.top).height();
                    g.top.height(topHeight);
                    g.middleTop = topHeight + p.space;
                }
...
(function($) {
    ...
    $.ligerui.getPopupFn = function(p) {
        ...
        function show() {
            ...
            //        click: function () {
            //            grid.query({
            //                filters: condition.toSearchFilters()
            //            });
...
(function($) {
    ...
    $.ligerui.controls.Portal.ligerExtend($.ligerui.core.UIComponent, {
        ...
        add: function(e) {
            ...
            var gColumn = g.rows[rowIndex].columns[columnIndex],
                pColumn = p.rows[rowIndex].columns[columnIndex],
            ...
            if ($.fn.ligerDrag && p.draggable) {
                jpanel.addClass("l-panel-draggable").ligerDrag({
                    ...
                    onDrag: function(current, e) {
                        ...
                        var height = jpanel.height(),
                            width = jpanel.width(),
                            offset = jpanel.offset();
                        var centerX = offset.left + width / 2,
                            centerY = offset.top + 10;
                        var panels = g.getPanels(),
                            emptyColumns = g.getEmptyColumns();
                        ...
                        function positionIn(panel, column, x, y) {
                            ...
                            var left = jelement.offset().left,
                                top = jelement.offset().top;
                            var diff = 3;
    ...
    $.ligerui.controls.RadioList.ligerExtend($.ligerui.controls.Input, {
        ...
        refresh: function() {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                out.push("<td><input type='radio' name='" + name + "' value='" + val + "' id='" + id + "'" + (p.disabled ? "disabled='disabled'" : "") + "/><label for='" + id + "'>" + txt + "</label></td>");
            }
            if (appendRowStart) out.push('</tr>');
        ...
        _addClickEven: function() {
            var g = this,
                p = this.options;
            //选项点击
            g.radioList.on('click', ':radio', function(e) {
                var value = g.getValue();
                if (value) g.valueField.val(value);
                g.trigger("select", [value]);
            });
        }
    });
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        setTabButton: function() {
            ...
            if (sumwidth > mainwidth) {
                ...
                g.tab.links.ul.animate({ left: 0 }, 100);
        ...
        moveToPrevTabItem: function(tabid) {
            ...
            for (var i = 0, l = tabItems.length; i < l; i++) {
                ...
                } else if ($(tabItems[i + 1]).offset().left >= prevBtnLeft) {
            ...
            g.tab.links.ul.animate({ left: -1 * left }, 100);
            return true;
        },
        //切换到下一个tab
        moveToNextTabItem: function(tabid) {
            ...
            for (var i = 0, l = tabItems.length; i < l; i++) {
                ...
                } else if (end > nextBtnOffset.left) {
        ...
        overrideTabItem: function(targettabid, options) {
            ...
            //如果不存在
            if (!g.isTabItemExist(targettabid)) {
                return;
            } else {
                if (g.itemCloseEvents[targettabid]) {
                    g.itemCloseEvents[targettabid].call(window);
                    delete g.itemCloseEvents[targettabid];
                }
            }
            ...
            if (options.onClose) {
                g.itemCloseEvents[tabid] = options.onClose;
            }
        },
        //设置页签项标题
        ...
        selectTabItem: function(tabid) {
            ...
            var $tabitem = $("li[tabid='" + tabid + "']", g.tab.links.ul),
                $itemContent = $("> .l-tab-content-item[tabid='" + tabid + "']", g.tab.content);
            $itemContent.show().siblings().hide();
            $tabitem.addClass("l-selected").siblings().removeClass("l-selected");
            g.moveToTabItem(tabid);
            g._tabContentResize($itemContent);
            g.trigger('afterSelectTabItem', [tabid]);
        },
        _tabContentResize: function($itemContent) {
            var $iframe = $('iframe', $itemContent);
            if ($iframe.length > 0 && $iframe.get(0).contentWindow && $iframe.get(0).contentWindow.$) {
                $iframe.get(0).contentWindow.$($iframe.get(0).contentWindow).resize();
            } else {
        ...
        moveToLastTabItem: function() {
            ...
            $("li", g.tab.links.ul).each(function() {
                tabitem = $(this);
                sumwidth += tabitem.width() + parseInt(tabitem.css("marginLeft")) +
                    parseInt(tabitem.css("marginRight"));
            });
            var mainwidth = g.tab.width();
            if (sumwidth > mainwidth) {
                var btnWitdth = $(".l-tab-links-right", g.tab.links).width();
                g.tab.links.ul.animate({ left: -1 * (sumwidth - mainwidth + btnWitdth + parseInt(tabitem.css("marginLeft")) + parseInt(tabitem.css("marginRight"))) }, 100);
            }
        },
        //判断tab是否存在
        ...
        _setTabText: function(tabitem, text) {
            var titles = [];
            tabitem.siblings().each(function(i, item) {
                titles.push($("a", item).text());
            });
            var title = text,
                index = 1,
                flag = true;
            ...
            $("a", tabitem).text(title).attr("title", title);
        },
        //增加一个tab
        ...
        removeTabItem: function(tabid) {
            ...
            if (g.trigger('beforeRemoveTabItem', [tabid]) == false)
                return false;
            var tabItem = $("li[tabid='" + tabid + "']", g.tab.links.ul);
            var currentIsSelected = tabItem.hasClass("l-selected");
            if (currentIsSelected) {
                var tempTabItem = tabItem.prev();
                if (tempTabItem.length == 0) {
                    tempTabItem = tabItem.next();
                }
                if (tempTabItem.length > 0) {
                    g.selectTabItem(tempTabItem.attr('tabid'));
                }
            }
            ...
            if (g.setTabButton()) {
                ...
                var g = this,
                    p = this.options;
                var tabItem = $("> li:last", g.tab.links.ul),
                    nextBtn = $(".l-tab-links-right", g.tab);
                var nextBtnOffset = nextBtn.offset(),
                    offset = tabItem.offset();
                if (nextBtnOffset.left > offset.left + tabItem.outerWidth()) {
                    g.moveToLastTabItem();
                }
            }
            g.trigger('afterRemoveTabItem', [tabid]);
            if (g.itemCloseEvents[tabid]) {
                g.itemCloseEvents[tabid].call(window);
                delete g.itemCloseEvents[tabid];
            }
        },
        ...
        onResize: function() {
            ...
            if (g.tab.parent()[0].tagName.toLowerCase() == "body") {
                ...
                g.height = p.heightDiff + windowHeight * parseFloat(p.height.replace('%', '')) * 0.01 - 1;
            } else {
                g.height = p.heightDiff + (g.tab.parent().height() * parseFloat(p.height.replace('%', '')) * 0.01) - 1;
            }
...
(function($) {
    ...
    $.ligerui.controls.TextBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _init: function() {
            ...
            if (p.onEnterKeydown && !p.onKeydown) {
                p.onKeydown = function(e) {
                    if (e.keyCode == kad.ui.keyCodes.keyEnter) {
                        p.onEnterKeydown(e);
                    }
                }
            }
        ...
        _render: function() {
            ...
            if (p.maxlength != null) {
                g.inputText.attr("maxlength", p.maxlength);
            }
        ...
        _setValue: function(value) {
            ...
            */
            ...
            if (p.nullText && !p.disabled && !value) {
                g.inputText.addClass("l-text-field-null").val(p.nullText);
            } else if (value) {
                if (p.trim) {
                    value = $.trim(value);
                }
                if (value && (p.digits || p.number)) {
                    value = parseFloat(value.toString().replace(/[^-\.\d]/g, ""));
                    if (isNaN(value)) {
                        value = "";
                    } else {
                        value = p.digits ? value.toFixed(0) : value.toFixed(p.precision);
                    }
                }
            }
            g.inputText.val(value);
...
(function($) {
    ...
    $.ligerui.controls.ToolBar.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _init: function() {
            $.ligerui.controls.ToolBar.base._init.call(this);
            var g = this,
                p = this.options;
        },
        _render: function() {
        ...
        _setItems: function(items) {
            ...
            $(items).each(function(i, item) {
                //权限验证
                if (item.id && p.permission && p.permission.items && p.permission.items[item.id] && !p.permission.items[item.id].right) {
                    return;
                }
                g.addItem(item);
            });
        ...
        setEnabled: function(item) {
            this._changeDisabled(item, true);
        },
        setDisabled: function(item) {
            this._changeDisabled(item, false);
            //var g = this, p = this.options;
            //$("> .l-toolbar-item[toolbarid=" + itemid + "]", g.toolBar).addClass("l-toolbar-item-disable");
        },
        _changeDisabled: function(item, disabled) {
            var g = this,
                p = this.options;
            var methodName = disabled ? 'removeClass' : 'addClass';
            if (item == null) {
                //修改所有按钮
                $("> .l-toolbar-item", g.toolBar)[methodName]("l-toolbar-item-disable");
            } else if (typeof(item) == "string") {
                $("> .l-toolbar-item[toolbarid=" + item + "]", g.toolBar)[methodName]("l-toolbar-item-disable");
            } else if (typeof(item) == "object") {
                if (item.constructor == Array) {
                    for (var i = 0; i < item.length; i++) {
                        g._changeDisabled(item[i], disabled);
                    }
                }
            }
        },
...
(function($) {
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        search: function(text, onlyLeaf) {
            var g = this;
            if (!g.orginalData) {
                g.orginalData = g.data;
            }
            var cloneData = g._cloneTreeData(g.orginalData);
            if (text) {
                g._filter(cloneData, text, onlyLeaf);
            }
            g._setData(cloneData);
            g.expandAll();
        },
        _filter: function(data, text, onlyLeaf) {
            var g = this,
                p = this.options,
                i = 0;
            while (i < data.length) {
                ...
                if (item.children && item.children.length > 0) {
                    if (!g._filter(item.children, text, onlyLeaf) && !onlyLeaf && item[p.textFieldName].indexOf(text) < 0) {
                        data.splice(i, 1);
                        continue;
                    }
                } else if (item[p.textFieldName].indexOf(text) < 0) {
                    data.splice(i, 1);
                    continue;
                }
                i++;
        ...
        _cloneTreeData: function(data) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                item.treedataindex = null;
                item[p.statusName] = null;
                var children = this._cloneTreeData(data[i].children);
        ...
        getNodeDom: function(nodeParm) {
            ...
            } else if (typeof(nodeParm) == "object" && 'target' in nodeParm) {
        ...
        selectNode: function(selectNodeParm) {
            ...
            if (typeof(selectNodeParm) == "function") {
                ...
                if (selectNodeParm.constructor == Array) {
                    clause = function(data) {
                        if (!data[p.idFieldName]) return false;
                        return $.inArray(data[p.idFieldName].toString(), selectNodeParm) >= 0;
                    };
                } else {
            ...
            $("li", g.tree).each(function() {
                ...
                if (clause(treenodedata, treedataindex)) {
                    ...
                    var treeitembody = $(">div:first", treeitem);
                    if (p.checkbox) {
                        if ($(".l-checkbox", treeitembody).hasClass("l-checkbox-checked")) {
                            g.cancelSelect(this);
                        }
                    } else if (treeitembody.hasClass("l-selected")) {
                        g.cancelSelect(this);
                    }
                }
            });
        ...
        setCheckEnabled: function() {
            this.checkDisabled = false;
            $(".l-checkbox", this.tree).removeClass('l-checkbox-disabled');;
        },
        setCheckDisabled: function() {
            this.checkDisabled = true;
            $(".l-checkbox", this.tree).addClass('l-checkbox-disabled');;
        },
        //递归设置父节点的状态
